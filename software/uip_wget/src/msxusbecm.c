/*
;
; msxusbecm.c - connection to the MSX USB Unapi driver.
...
unsigned int  msxusbecm_read ()
{
    ...
    if (regs.Bytes.A!=0x14) {
        ...
        //printf ("Read %d bytes\r\n",regs.Words.BC);
        #if USB_DEBUG
        printf ("<< dst: %x:%x:%x:%x:%x:%x - src: %x:%x:%x:%x:%x:%x - type: %x - length: %d bytes\r\n",
            ...
            *(uip_buf+6),*(uip_buf+7),*(uip_buf+8),*(uip_buf+9),*(uip_buf+10),*(uip_buf+11), // src
            ((*(uip_buf+12))<<8)+((*(uip_buf+13))),regs.Words.BC);// EtherType
        #endif 
    }
...
void msxusbecm_send ()
{
    ...
    printf (">> dst: %x:%x:%x:%x:%x:%x - src: %x:%x:%x:%x:%x:%x - type: %x - length: %d bytes\r\n",
            *(uip_buf+0),*(uip_buf+1),*(uip_buf+2),*(uip_buf+3),*(uip_buf+4),*(uip_buf+5), // dst
            *(uip_buf+6),*(uip_buf+7),*(uip_buf+8),*(uip_buf+9),*(uip_buf+10),*(uip_buf+11), // src
            ((*(uip_buf+12))<<8)+((*(uip_buf+13))),uip_len);// EtherType
    #endif
    regs.Words.HL = (uint16_t) &uip_buf;
