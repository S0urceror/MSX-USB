...
void ch376_set_usb_address(const uint8_t device_address)
{
    write_command(CMD_SET_USB_ADDR);
    write_data(device_address);
}

uint8_t ch376_control_transfer_request_descriptor(const uint8_t descriptor_type)
{
    uint8_t c376_status;

    write_command(CMD_GET_DESCR);
    write_data(descriptor_type);
    delay_ms(250);

    c376_status = ch376_wait_status();

    if (c376_status == CMD_RET_SUCCESS)
        return USB_ERR_OK;

    if (c376_status == USB_INT_SUCCESS)
        return USB_ERR_OK;

    if (c376_status == USB_INT_DISK_READ)
        return USB_ERR_DISK_READ;

    if (c376_status == USB_INT_DISK_WRITE)
        return USB_ERR_DISK_WRITE;

    if (c376_status == USB_INT_DISCONNECT) {
        ch376_set_usb_host_mode(5);
        return USB_ERR_NO_DEVICE;
    }

    if (c376_status == USB_INT_BUF_OVER)
...
uint8_t ch376_control_transfer_set_address(const uint8_t device_address)
{
    uint8_t c376_status;

    write_command(CMD_SET_ADDRESS);
    write_data(device_address);
